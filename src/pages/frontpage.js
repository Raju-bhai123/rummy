...
const InfoCard = ({ title, version, onClick }) => (
  <div
    className="flex flex-col sm:flex-row w-full space-y-4 cursor-pointer sm:space-y-0 sm:space-x-4 mb-5 p-4 border border-4 border-gray-600 mx-1 rounded bg-gray-200"
    onClick={() => onClick(title)} // Use the onClick passed as a prop
  >
    {/* Logo */}
    <div className="flex justify-center sm:justify-start flex-shrink-0">
      <img src={Logo} alt="Logo" className="w-14 h-14 sm:w-14 sm:h-14" />
    </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {versions.map((item, index) => (
              <InfoCard
                ...
                onClick={handleClick} // Pass handleClick as prop
              />
            ))}
          </div>
        </div>
